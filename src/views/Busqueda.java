...
public class Busqueda extends JFrame {
	...
	private huespedControlador huespedControl;
	...
	public Busqueda() {
		...
		this.huespedControl = new huespedControlador();
		...
		btnEliminar.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(filaReservas >= 0) {
					...
					int confirmar = JOptionPane.showConfirmDialog(null, "Deseas eliminar la reserva?");
					if(confirmar == JOptionPane.YES_OPTION) {
						String valor = tbReservas.getValueAt(filaReservas, 0).toString();
						reservasControl.eliminaReserva(Integer.valueOf(valor));
					...
					reserva = tbHuespedes.getValueAt(filaHuespedes, 0).toString();
					int confirmar = JOptionPane.showConfirmDialog(null, "Deseas eliminar la huesped?");
					if(confirmar == JOptionPane.YES_OPTION) {
						String valor = tbHuespedes.getValueAt(filaHuespedes, 0).toString();
						huespedControl.eliminarHuesped(Integer.valueOf(valor));
						JOptionPane.showMessageDialog(contentPane, "huesped eliminada con exito");
						limpiarTabla();
						mostrarTablaReservas();
						mostrarHuespedes();
					...
					JOptionPane.showMessageDialog(null, "Hubo un error eliminando");
				}
			}
		});
	...
	private void ActualizaReservas() {
	...
	private void ActualizaHuespedes() {
		...
		.ifPresentOrElse(filaHuespedes ->{
			...
			if(tbHuespedes.getSelectedColumn() == 0) {
				...
				this.huespedControl.actualizaHuesped(nombre, apellido, fechaNacimiento, nacionalidad, telefono, id_reserva, id);
				JOptionPane.showMessageDialog(this, String.format("Resgistro modificado con exito"));
			}			
						
		},()-> JOptionPane.showInternalMessageDialog(this, "Opcion no valida") );
	}
